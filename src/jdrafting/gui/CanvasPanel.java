...
public class CanvasPanel extends JPanel
{
	...
	{
		final List<JDraftingShape> shapes = getAllShapesAtCanvasPoint( mousePosition ); 
		if ( shapes.size() < 2 )
			return null;
		
		final Point2D logicMouse = getInverseTransform().transform( mousePosition, null );
		final JDraftingShape jdshape1 = shapes.get( 0 ),
							 jdshape2 = shapes.get( 1 );
		
		final List<Point2D> intersections = intersectionPoints( jdshape1, jdshape2 );

		return intersections // nearest intersection to mouse
			   .stream()
			   .min( ( point1, point2 ) -> Double.compare( point1.distanceSq( logicMouse ),
												 ...
												 Viewport orig, Viewport dest )
	{
		...
		{
			final AffineTransform transform = getTransform( orig, dest );
			
			Rectangle2D enclosure = getShapeCanvasBounds( exercise.getShapes().get(0), transform ); 
			
			for ( final JDraftingShape jdshape : exercise.getShapes() )
				enclosure = enclosure.createUnion( getShapeCanvasBounds( jdshape, transform ) );
			
			return getInverseTransform(transform).createTransformedShape(enclosure).getBounds2D();
		}
	...
	public static Rectangle2D getShapeCanvasBounds( JDraftingShape jdshape, 
													AffineTransform transform )
	{
		final Rectangle2D bounds = transform.createTransformedShape( 
												   jdshape.getShape().getBounds2D() ).getBounds2D();
		final float lineWidth = jdshape.getStroke().getLineWidth();
