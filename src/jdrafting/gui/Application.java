...
import jdrafting.gui.controller.actions.AreaIntersectionAction;
import jdrafting.gui.controller.actions.AreaSubstractAction;
import jdrafting.gui.controller.actions.AreaSymmetricDifferenceAction;
import jdrafting.gui.controller.actions.AreaUnionAction;
...
import jdrafting.gui.controller.actions.UndoRedoHistoryAction;
...
public class Application extends JFrame
{
	...
	// colors
	public static final String HTML_SHAPE_NAMES_COL = "#22DD22"; 
	// separators
	...
	public UndoableEditSupport undoRedoSupport;
	// GUI components
	public JWindow undoRedoWindow;
	...
	private void initUI()
	{
		...
		JDUtils.fixSize( buttonPointColor, 30, 30 );
		styleToolbar.add( Box.createHorizontalStrut( 3 ) );
		...
		JDUtils.fixSize( spinAngle, 70, 30 );
	...
	private void createActions()
	{
		...
		registerAction( new UndoRedoHistoryAction( this ) );
		...
		menuTools.addSeparator();
		// Area intersection
		menuTools.add( registerAction( new AreaIntersectionAction( this ) ) );
		// Area union
		menuTools.add( registerAction( new AreaUnionAction( this ) ) );
		// Area substract
		menuTools.add( registerAction( new AreaSubstractAction( this ) ) );
		// Area symmetric difference
		menuTools.add( registerAction( new AreaSymmetricDifferenceAction( this ) ) );
		menuTools.addSeparator();
		// Paint shape tool
		...
		actionbar.add( btnHistory = new JButton( actionMap.get( "\u25BC" ) ) );
		...
		actionbar.addSeparator( HSEP );
		JButton button;
		final Color bColor = new Color( 44, 33, 22 );  // color of exercise player buttons
		...
		actionbar.add( Box.createHorizontalStrut( 2 ) );		
		actionbar.addSeparator( HSEP );
		actionbar.add( toggleNames=new JToggleButton( actionMap.get( getLocaleText( "text" ) ) ) );
		...
		toolbar.addSeparator( HSEP );
		toolbar.add( actionMap.get(	getLocaleText( "area_intersection" ) ) );
		toolbar.add( actionMap.get(	getLocaleText( "area_union" ) ) );
		toolbar.add( actionMap.get(	getLocaleText( "area_substract" ) ) );
		toolbar.add( actionMap.get(	getLocaleText( "area_sym_diff" ) ) );
		toolbar.addSeparator( HSEP );
							 ...
							 Color color, Color fill, BasicStroke stroke, CompoundEdit transaction )
	{
		...
		if ( transaction == null )
			undoRedoSupport.postEdit( addEdit );
		else
	 ...
	 */
	public JDraftingShape addShapeFromIterator( PathIterator pit, String name, String description, 
												Color color, Color fill, BasicStroke stroke )
	{
	...
	{
		// remove shape
		final int index = getExercise().removeShape( jdshape );
		shapeList.getModel().removeElement( jdshape );
		
		// undo/redo system
		final UndoableEdit remove = new EditRemoveShapeFromExercise( jdshape, index );
		if ( transaction == null )
			undoRedoSupport.postEdit( remove );
		else
	...
	{
		public int size() { return this.  edits.size(); }

		@Override
		public UndoableEdit editToBeRedone() { return super.editToBeRedone(); }
		
		@Override
		public UndoableEdit editToBeUndone() { return super.editToBeUndone(); }
	...
	public void initializeUndoRedoSystem()
	{
		...
		undoRedoSupport = new UndoableEditSupport();
		undoRedoSupport.addUndoableEditListener( new UndoRedoListener() ); 
		refreshUndoRedo();
	}
	...
	public void refreshUndoRedo()
	{
		...
		action.putValue( Action.SHORT_DESCRIPTION, 
						 "<html>" + undoManager.getUndoPresentationName() + "</html>" );
		action = actionMap.get( getLocaleText( "redo" ) );
		action.setEnabled( undoManager.canRedo() );
		action.putValue( Action.SHORT_DESCRIPTION,
						 "<html>" + undoManager.getRedoPresentationName() + "</html>" );
	...
	public class EditAddShapeToExercise extends AbstractUndoableEdit
	{
		...
		{
			return getLocaleText( "add" ) + " " + jdshape.toShortString();
		}
	}
	...
	public  class EditRemoveShapeFromExercise extends AbstractUndoableEdit
	{
		...
		public String getPresentationName()
		{
			return getLocaleText( "remove" ) + " " + jdshape.toShortString();
		}
	}
