...
public class RegularPolygonListener extends AbstractCanvasMouseListener
{
		...
		else
		{
			if ( addMore() )  // add complete transaction
			{
				...
				CompoundEdit transaction = new CompoundEdit() {
					...
					{
						return "Redo regular polygon";
					}
					@Override
					public String getUndoPresentationName()
					{
						return "Undo regular polygon";
					}
				};
	...
	private Map<String,Object> getPolygon( Point2D logicMouse )
	{
		...
		double radius = app.isUsingRuler() 
						? app.getDistance() 
						: center.distance( logicMouse );
		Point2D center = this.center != null ? this.center : logicMouse;
		
		// create polygon
		Path2D polygon = new Path2D.Double();
		double sideAng = 2 * Math.PI / n;
		double offsetAng = Math.PI * ( 3. / 2. - 1. / n ) % sideAng;
