...
public class ArrowListener extends AbstractCanvasMouseListener
{
	...
	private Path2D getArrow( Point2D logicMouse )
	{
		...
		double afactor = 1.075, dfactor = Math.log( Math.E + dist / 1000 ) * 4.;
		double ang = vectorArg( vector( start, end ) );
		Path2D arrow = new Path2D.Double();
		
		arrow.moveTo( start.getX(), start.getY() );
		arrow.lineTo( end.getX(), end.getY() );
		Point2D aux = 
			pointRelativeToCenter( end, ang + PI * afactor, dist / dfactor );
		...
		{
			arrow.moveTo( start.getX(), start.getY() );
			aux = pointRelativeToCenter( 
							start, ang + PI * ( afactor - 1 ), dist / dfactor );
			arrow.lineTo( aux.getX(), aux.getY() );
			arrow.moveTo( start.getX(), start.getY() );
			aux = pointRelativeToCenter( 
							start, ang - PI * ( afactor - 1 ), dist / dfactor );
