...
public class JDMath
{
	...
	/**
	 * Get centroid of a point set
	 * @param points vertex set
	 * @return the centroid
	 */
	public static Point2D centroid( Iterable<Point2D> points )
	{
		Point2D center = new Point2D.Double();
		int size = 0;
		for ( Point2D p : points )
		{
			center = sumVectors( center, p );
			size++;
		}
		
		return new Point2D.Double( center.getX() / size, center.getY() / size );
	}
	...
	/**
	 * Solve cuadratic equation ax^2+bx+c=0
	 * @param a param a
	 * @param b param b
	 * @param c param c
	 * @return 2D array with two solutions
	 */
	public static double[] solveQuadratic( double a, double b, double c )
	{
		double disc = Math.sqrt( b * b - 4 * a * c );
		
		return new double[] { ( -b + disc ) / ( 2 * a ), 
							  ( -b - disc ) / ( 2 * a ) };
	}
