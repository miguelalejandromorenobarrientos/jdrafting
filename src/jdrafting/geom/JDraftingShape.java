...
public class JDraftingShape implements Serializable
{
		...
		while ( !pit.isDone() )
		{
			switch( pit.currentSegment( coords ) )
			{
				case PathIterator.SEG_LINETO:
					segments.add( 
							new Line2D.Double( oldX, oldY, oldX = coords[0], oldY = coords[1] ) );
					break;
				case PathIterator.SEG_MOVETO:
					pit.currentSegment( open );
					oldX = coords[0];
					oldY = coords[1];
					break;
				case PathIterator.SEG_QUADTO:
					segments.add( 
							new Line2D.Double( oldX, oldY, oldX = coords[2], oldY = coords[3] ) );
					break;
				case PathIterator.SEG_CUBICTO:
					segments.add( 
							new Line2D.Double( oldX, oldY, oldX = coords[4], oldY = coords[5] ) );
					break;
				case PathIterator.SEG_CLOSE:
					segments.add( new Line2D.Double( oldX, oldY, open[0], open[1] ) );
					break;
	...
	public String toString()
	{
		...
		sb.append( "[name=" )
		  .append( ( getName() != null && getName().length() > 0 ? getName() : "(unnamed)" ) )
		  .append( getDescription() != null && getDescription().length() > 0 
		  		   ? ",description=" + getDescription() 
		  		   : "" )
		  .append( ",color=" ).append( getColor() )
		  .append( ",fill=" ).append( elvis( getFill(), "" ) )
		  .append( ",isText=" ).append( isText() )
		  .append( ",font=" ).append( elvis( getFont(), "" ) )
		  .append( ",length=" ).append( JDMath.length( getShape(), null ) ) 
		  .append( ",path=" ).append( JDUtils.pathToString( getShape() ) )
		  .append( "]" );
